...
add_action( 'after_switch_theme', 'emeon_switch_theme'   );
add_action( 'load-customize.php', 'emeon_load_customize' );
add_action( 'template_redirect',  'emeon_preview'        );
 ...
 * @return string
 */
function emeon_compat_message() {
	if ( version_compare( $GLOBALS['wp_version'], EMEON_MIN_WP_VERSION, '<' ) ) {

		return sprintf(
		/* translators: %1$s: Theme name, %2$s: the required WordPress version, %3$s: user's current version */
			__( '%1$s theme requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'emeon' ),
			EMEON_OPTIONS_NAME_PART,
			EMEON_MIN_WP_VERSION,
			$GLOBALS['wp_version']
		);

	} elseif ( version_compare( PHP_VERSION, EMEON_MIN_PHP_VERSION, '<' ) ) {

		return sprintf(
		/* translators: %1$s: Theme name, %2$s: required PHP version, %3$s: current PHP version */
			__( '%1$s theme requires at least PHP version %2$s. You are running version %3$s. Please upgrade and try again.', 'emeon' ),
			EMEON_OPTIONS_NAME_PART,
			EMEON_MIN_PHP_VERSION,
			PHP_VERSION
		);
	}
 ...
 * @return void
 */
function emeon_switch_theme( $old_name ) {
 ...
 * @return void
 */
function emeon_load_customize() {

	wp_die( esc_html( emeon_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function emeon_preview() {

	if ( isset( $_GET['preview'] ) ) { // WPCS: CSRF ok.
		wp_die( esc_html( emeon_compat_message() ) );
	}
}

...
if(!function_exists( 'emeon_get_post_meta_data' )){
    /**
     * Returns post meta data like date, author, comments number and categories
     *
     * @since 2.3.0
     */
    function emeon_get_post_meta_data() {
        global $emeon_a13;

        $types      = emeon_what_page_type_is_it();
	    ...
	    //return date
        if(
            ($post && $emeon_a13->get_option( 'post_date') === 'on')
            ||
            ($post_list && $emeon_a13->get_option( 'blog_date') === 'on')
        ){
            $return .= emeon_posted_on();
        }

	    //return updated date
        if(
            ($post && $emeon_a13->get_option( 'post_date') === 'updated')
            ||
            ($post_list && $emeon_a13->get_option( 'blog_date') === 'updated')
        ){
            $return .= esc_html__( 'Updated: ', 'emeon' ).emeon_modified_on();
        }
        ...
        elseif(
		    ($post && $emeon_a13->get_option( 'post_author') === 'on')
		    ||
		    ($post_list && $emeon_a13->get_option( 'blog_author') === 'on')
	    ){
		    $return .= emeon_posted_by_author();
	    }

        //return comments number
        if(
            ($post && $emeon_a13->get_option( 'post_comments') === 'on')
            ||
            ($post_list && $emeon_a13->get_option( 'blog_comments') === 'on')
        ){
            $return .= emeon_post_comments();
        }

        //Categories
        if(
            ($post && $emeon_a13->get_option( 'post_cats') === 'on')
            ||
            ($post_list && $emeon_a13->get_option( 'blog_cats') === 'on')
        ){
...
if(!function_exists( 'emeon_post_meta_data' )){
    /**
     * Outputs post meta data like date, author, comments number and categories
     *
     * @since 2.3.0
     */
    function emeon_post_meta_data() {
        echo wp_kses_post(emeon_get_post_meta_data());
    }
}


if(!function_exists('emeon_posted_on')){
	/**
	 * get HTML for date of post
     * @return string
     */
    function emeon_posted_on() {
        return '<time class="entry-date published updated" datetime="'.esc_attr( get_the_date( 'c' ) ).'" itemprop="datePublished">'.get_the_date().'</time> ';
    }
}


if(!function_exists('emeon_modified_on')){
	/**
	 * get HTML for date of post
     * @return string
     */
    function emeon_modified_on() {
...
if(!function_exists('emeon_posted_by_author')){
	/**
	 * Author of post
	 * @return string
	 */
	function emeon_posted_by_author() {
        /** @noinspection HtmlUnknownAttribute */
        return
            /* translators: %s - author name */
			sprintf(  esc_html__( 'by %s ', 'emeon' ),
                sprintf('<a class="vcard author" href="%1$s" title="%2$s" %4$s><span class="fn" %5$s>%3$s</span></a>',
				    esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )),
                    /* translators: %s - author name */
				    sprintf( esc_attr__( 'View all posts by %s', 'emeon' ), get_the_author() ),
				    get_the_author(),
                    emeon_get_schema_args('author'),
                    emeon_get_schema_args('name')
                )
...
if(!function_exists('emeon_post_comments')){
	/**
	 * comments link
	 *
     * @return string HTML
     */
    function emeon_post_comments() {
...
if(!function_exists('emeon_under_post_content')){
    function emeon_under_post_content() {
        global $emeon_a13;
        ...
        wp_link_pages( array(
                'before' => '<div id="page-links"><span class="page-links-title">'. esc_html__( 'Pages: ', 'emeon' ).'</span>',
                'after'  => '</div>'
	        )
        );
        ...
        //Tags under content
        if(
            ($post && $emeon_a13->get_option( 'post_tags' ) === 'on')
            ||
            ($post_list && $emeon_a13->get_option( 'blog_tags' ) === 'on')
        ){
...
if(!function_exists('emeon_author_info')){
	/**
     * Displays author info in posts(if enabled)
     */
    function emeon_author_info() {
        global $emeon_a13;

        $author_description =  get_the_author_meta( 'description' );

        if( ( strlen($author_description) > 0 ) && $emeon_a13->get_option( 'post_author_info' ) === 'on'): ?>
            <div class="about-author comment"<?php emeon_schema_args('author_box'); ?>>
                <div class="comment-body">
                    <?php $author_ID = get_the_author_meta( 'ID' );
                        echo '<a href="'.esc_url( get_author_posts_url($author_ID) ).'" class="avatar"'.esc_attr(emeon_get_schema_args('url')).'>'.get_avatar( $author_ID, 90 ).'</a>';
                        echo '<strong class="comment-author"><span class="author-name" '.esc_attr(emeon_get_schema_args('name')).'>'.get_the_author().'</span>';
...
if(!function_exists('emeon_posts_navigation')){
	/**
     * Displays navigation to next and previous post
     */
    function emeon_posts_navigation() {
        global $emeon_a13;

        if($emeon_a13->get_option( 'post_navigation' ) === 'on'){
            //posts navigation
            ...
            if($is_prev || $is_next){
                ...
                add_filter( 'emeon_post_navigation_image_size', function(){
                    return array(245,245);
                });
                 *
                 */
                $image_sizes = apply_filters( 'emeon_post_navigation_image_size', array( 245, 100 ) );

                if($is_prev){
                    $id = $prev_post->ID;
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item prev">'
                         .'<span><i class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'"></i> '.esc_html__( 'Previous article', 'emeon' ).'</span>'
                         .'<span class="title">'.esc_html( $prev_post->post_title ).'</span>'
                         .'<span class="image">'.esc_attr(emeon_make_post_image( $id, $image_sizes )).'</span>'
                         .'</a>';
                }
                if($is_next){
                    $id = $next_post->ID;
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item next">'
                         .'<span>'.esc_html__( 'Next article', 'emeon' ).' <i class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'"></i></span>'
...
if(!function_exists('emeon_excerpt_length')){
    /**
     ...
     */
    function emeon_excerpt_length($length) {
        ...
        global $emeon_a13;
        return intval( $emeon_a13->get_option( 'blog_excerpt_length') );
    }
}
add_filter( 'excerpt_length', 'emeon_excerpt_length' );


if(!function_exists('emeon_new_excerpt_more')){
    /**
     ...
     * @return string HTML
     */
    function emeon_new_excerpt_more($link) {
        $ajax_for_vc_grid = defined( 'DOING_AJAX' ) && DOING_AJAX && isset($_POST['action']) && ($_POST['action'] === 'vc_get_vc_grid_data');
        ...
        global $post, $emeon_a13;
        if( $emeon_a13->get_option( 'blog_read_more', 'on' ) === 'off' ){
            return '';
        }

        //return read more
        return '&hellip;
 <p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'emeon' ).'</a></p>';
    }
}
add_filter( 'excerpt_more', 'emeon_new_excerpt_more' );



if(!function_exists('emeon_has_excerpt_read_more')){
	/**
	 * Adds read more when excerpt is provided by user
	 *
     * @return string HTML
     */
    function emeon_has_excerpt_read_more($content) {
        global $post, $emeon_a13;

        if( $emeon_a13->get_option( 'blog_read_more', 'on' ) === 'on' && has_excerpt() ){
            $content .= '<p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'emeon' ).'</a></p>';
        }

        return $content;
    }
}
add_filter( 'the_excerpt', 'emeon_has_excerpt_read_more' );


if(!function_exists('emeon_read_more_new_line')){
	/**
	 ...
	 * @return string HTML
	 */
    function emeon_read_more_new_line($link) {
        return '<p>'.$link.'</p>';
    }
}
add_filter( 'the_content_more_link', 'emeon_read_more_new_line' );



if(!function_exists('emeon_comments_navigation')){
	/**
     * Comments navigation
     */
    function emeon_comments_navigation() {
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
            ?>
            <nav class="navigation comment-navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'emeon' ); ?></h2>
                <div class="nav-links">
                    <?php
                    if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'emeon' ) ) ) :
                        printf( '<div class="nav-previous">%s</div>', wp_kses_post( $prev_link ) );
                    endif;

                    if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'emeon' ) ) ) :
...
if(!function_exists('emeon_daoon_chat_post')){
    /**
     ...
     * @return string
     */
    function emeon_daoon_chat_post($content) {
...
if(!function_exists('emeon_display_items_from_query_post_list')) {
    /**
     * @param bool|WP_Query $query
     * @param array         $args
     */
    function emeon_display_items_from_query_post_list($query = false, $args = array()){
        global $emeon_a13;
        ...
        $default_args = array(
            'columns'         => $emeon_a13->get_option( 'blog_brick_columns' ),
            'max_width'       => $emeon_a13->get_option( 'blog_bricks_max_width' ),
            'margin'          => $emeon_a13->get_option( 'blog_brick_margin' ),
            'filter'          => false,
        ...
        if($args['filter']){
            ...
            emeon_make_post_grid_filter($terms, 'posts-filter', $args['default_filter'], $args['all_filter']);
        }
        ...
        if ( ! $query->have_posts() ):
            ...
            <div class="formatter">
                <div class="real-content empty-blog">
                    <?php
                    if( !is_search() ){
                        echo '<p>'.esc_html__( 'Apologies, but no results were found for the requested archive.', 'emeon' ).'</p>';
                    }
            ...
            $posts_classes = '';
            $posts_layout_class = $emeon_a13->get_option( 'blog_post_look');
            ...
            if($post_list_page){
                ...
                $lazy_load_mode   = $emeon_a13->get_option( 'blog_lazy_load_mode' );
            ...
            while ( $query->have_posts() ) : $query->the_post();
                ...
                //size of brick
                $brick_size = ($posts_layout_class === 'horizontal') ? 1 : $emeon_a13->emeon_get_meta( '_brick_ratio_x' );
                ...
                emeon_schema_args('creative');
                echo '>';

                    if(post_password_required()){
                        emeon_top_image_video($link_it, array_merge($args, array('page_type' => 'blog_type', 'post_grid' => true ) ) );
                        ?>
                        <div class="formatter">
                            <?php the_title('<h2 class="post-title entry-title"><a href="'. esc_url( get_permalink() ) . '"><span class="fa fa-lock"></span>', '</a></h2>'); ?>
                            <div class="real-content">
                                <p><?php esc_html_e( 'To view it please enter your password below', 'emeon' ); ?></p>
                                <?php echo wp_kses_post(emeon_password_form()); //escaped on creation  ?>
                            </div>
                        </div>
                        <?php
                    }
            ...
            echo '</div></div>';
            if(!$ajax_call){
                emeon_post_list_individual_look($id, $args);
            }
        endif;
    }
}
 ...
 */
function emeon_remove_hentry( $class ) {
    $class = array_diff( $class, array( 'hentry' ) );
    return $class;
}
add_filter( 'post_class', 'emeon_remove_hentry' );



if ( ! function_exists( 'emeon_next_posts_link_class' ) ) {
	/**
	 * Adding class for compatibility with Wp-paginate plugin + infinite scroll configuration
	 *
	 * @return string
	 */
	function emeon_next_posts_link_class() {
		return 'class="next"';
	}
}

if ( ! function_exists( 'emeon_prev_posts_link_class' ) ) {
	/**
	 * Adding class for compatibility with Wp-paginate plugin + infinite scroll configuration
	 *
	 * @return string
	 */
	function emeon_prev_posts_link_class() {
		return 'class="prev"';
	}
}
add_filter( 'next_posts_link_attributes', 'emeon_next_posts_link_class' );
add_filter( 'previous_posts_link_attributes', 'emeon_prev_posts_link_class' );



if(!function_exists('emeon_post_list_individual_look')){
    /**
     ...
     */
    function emeon_post_list_individual_look($id, $args){
        $css = '';

        $max_width        = (int) $args['max_width'];
        $max_width        = esc_html( emeon_make_css_rule( 'max-width', $max_width, '%spx' ) );
...
if(!function_exists('emeon_get_schema_args')){
    /**
     ...
     */
    function emeon_get_schema_args( $for ){
        $prop = '';
        $scope = false;
        $type = '';
        ...
        $attributes = apply_filters( 'emeon_schema_args', $attributes, $for );
...
if(!function_exists('emeon_schema_args')){
    /**
     ...
     * @return      string - string of HTML attributes
     */
    function emeon_schema_args( $for ) {
        echo wp_kses_post(emeon_get_schema_args( $for ));
    }
}

if(!function_exists('emeon_remove_wpseo')){
    /**
...
add_action( 'template_redirect', 'emeon_remove_wpseo' );
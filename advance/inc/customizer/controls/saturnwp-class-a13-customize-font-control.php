 ...
 *
 */
class Emeon_A13_Customize_Font_Control extends WP_Customize_Control {
	 ...
	 *
	 * @see Emeon_A13_Customize_Font_Control::content_template()
	 */
	...
	public function content_template() {
		...
		var id = data.settings.default,
			...
			standard_fonts = EmeonA13FECustomizerControls.standard_fonts,
			google_fonts = EmeonA13FECustomizerControls.google_fonts,
			readabe_variants = EmeonA13FECustomizerControls.human_font_variants,
		...
		<div class="customize-control-content">
			<label for="{{ id }}" class="outside"><?php esc_html_e( 'Font Family', 'emeon' ); ?></label>
			<select id="{{ id }}">
				<# if ( typeof standard_fonts === 'object' ) { #>
					<optgroup label="<?php echo esc_attr( __( 'Standard Fonts', 'emeon' )); ?>">
				...
				<# if ( typeof google_fonts === 'object' ) { #>
					<optgroup label="<?php echo esc_attr( __( 'Google Webfonts', 'emeon' )); ?>">'
			...
			<# if(typeof google_fonts[selected_font] !== 'undefined'){ #>
				<fieldset class="font-subsets checkboxes">
					<legend><?php esc_html_e( 'Font Subsets', 'emeon' ); ?></legend>
				...
				<fieldset class="font-variants checkboxes">
					<legend><?php esc_html_e( 'Font Weights & Styles', 'emeon' ); ?></legend>
			...
			<fieldset class="font-spacing">
				<div>
					<label class="outside" for="{{ id_prefix }}word-spacing"><?php esc_html_e( 'Word Spacing', 'emeon' ); ?></label>
					<input id="{{ id_prefix }}word-spacing" type="number" name="word-spacing" value="{{ selected_word_spacing }}" /><span class="input-unit">px</span>
				</div>
				<div>
					<label class="outside" for="{{ id_prefix }}letter-spacing"><?php esc_html_e( 'Letter Spacing', 'emeon' ); ?></label>
